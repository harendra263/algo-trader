...
class EClient(object):
    ...
    def startApi(self):
        """  Initiates the message exchange between the ib_client application and
        the TWS/IB Gateway. """
    ...
    def connect(self, host, port, clientId):
        ...
        clientId:int - A number used to identify this ib_client connection. All
            orders placed/modified from this ib_client will be associated with
            this ib_client identifier.

            Note: Each ib_client MUST connect with a unique clientId."""
    ...
    def reqOpenOrders(self):
        ...
        placed from this ib_client. Each open order will be fed back through the
        openOrder() and orderStatus() functions on the EWrapper.

        Note:  The ib_client with a clientId of 0 will also receive the TWS-owned
        open orders. These orders will be associated with the ib_client and a new
    ...
    def reqAutoOpenOrders(self, bAutoBind:bool):
        ...
        be implicitly associated with the ib_client. When a new TWS order is
        created, the order will be associated with the ib_client, and fed back
        through the openOrder() and orderStatus() functions on the EWrapper.

        Note:  This request can only be made from a ib_client with clientId of 0.

        bAutoBind: If set to TRUE, newly created TWS orders will be implicitly
        associated with the ib_client. If set to FALSE, no association will be
        made."""
    ...
    def reqAccountUpdates(self, subscribe:bool, acctCode:str):
        ...
        subscribe:bool - If set to TRUE, the ib_client will start receiving account
            and Portfoliolio updates. If set to FALSE, the ib_client will stop
    ...
    def reqExecutions(self, reqId:int, execFilter:ExecutionFilter):
        ...
        filter criteria are downloaded to the ib_client via the execDetails()
